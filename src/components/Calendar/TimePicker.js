...
import {scale, moderateScale} from 'utils/Normalize';
import {Shadow} from 'react-native-shadow-2';
import moment from 'moment';
import {GetBlocks} from 'utils/GetBlocks.js';
import {verticalScale} from 'utils/Normalize.js';
  ...
  return (
    <View style={{paddingHorizontal: 10}}>
      ...
      {Object.keys(day_parts).map(part => {
        console.log(day_parts[part].length);
        if (day_parts[part].length) {
          return (
            <View style={{marginTop: verticalScale(10)}}>
              ...
              {GetBlocks(day_parts[part], 3).map(block => (
                <View style={{flexDirection: 'row', marginTop: 5}}>
                  {block.map((el, i) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        entry.setDateAndTime(selected_date, el);
